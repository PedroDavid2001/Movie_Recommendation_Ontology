...
# =================================================================
# lista de amizades
alice.friend_Of = [bob, carol, david]
bob.friend_Of = [alice, eve]
carol.friend_Of = [alice, frank]
david.friend_Of = [alice, grace]
eve.friend_Of = [bob, henry]
frank.friend_Of = [carol, ivy]
grace.friend_Of = [david, jack]
...
p3.mediatesUser = [bob]
p3.mediatesGenre = [action, comedy]
...
SELECT DISTINCT ?friend ?likedGenre ?movie
WHERE {{
    <{usuario.iri}> :friend_Of ?friend .
    
    ?pref rdf:type :Positive_Genre_Preference .
    ?pref :mediatesUser ?friend .
    ?pref :mediatesGenre ?likedGenre .
    
    ?movie :belongs_To_Genre ?likedGenre .
}}
"""

resultados = list(default_world.sparql(query))

generos_amigos = defaultdict(set)
filmes_recomendados = set()

for friend, genre, movie in resultados:
    generos_amigos[friend.name].add(genre.name)
    filmes_recomendados.add(movie.name)

print(f'Os amigos de {usuario.name} gostam de:\n')

for amigo, generos in sorted(generos_amigos.items()):
    print(
        f'{amigo} = '
        f'{{{", ".join(sorted(generos))}}}'
    )

print(f'\nEntão talvez {usuario.name} goste de:\n')

print(
    '{' +
    ', '.join(sorted(filmes_recomendados)) +
    '}'
)
