...
for (genre,) in default_world.sparql(query_likes):
...
for (genre,) in default_world.sparql(query_dislikes):
    disliked_genres.add(genre.name)
# =================================================================
# CONSULTA FINAL
...
WHERE {{
    ...
    ?prefPos rdf:type :Positive_Genre_Preference .
    ?prefPos :mediatesUser <{usuario.iri}> .
    ?prefPos :mediatesGenre ?likedGenre .

    ?movie :belongs_To_Genre ?likedGenre .

    FILTER NOT EXISTS {{

        ?prefNeg rdf:type :Negative_Genre_Preference .
        ?prefNeg :mediatesUser <{usuario.iri}> .
        ?prefNeg :mediatesGenre ?dislikedGenre .

        ?movie :belongs_To_Genre ?dislikedGenre .
    }}
}}
"""
...
recommended_movies = {
    movie.name
    for (movie,) in default_world.sparql(query_movies)
}

print(
    f"{usuario.name} gosta dos gêneros: "
    f"{{{', '.join(sorted(liked_genres))}}}"
)

print(
    f"{usuario.name} NÃO gosta dos gêneros: "
    f"{{{', '.join(sorted(disliked_genres))}}}"
)

print(
    f"Então vou recomendar: "
    f"{{{', '.join(sorted(recommended_movies))}}}"
)
